...
sub startAnimation()
    ...
    m.supportOpenGL = createObject("roDeviceInfo").GetGraphicsPlatform() = "opengl" 
    if m.supportOpenGL <> invalid and m.supportOpenGL 'in this case animation start depends from OpenGl support
...
function onKeyEvent(key as String, press as Boolean) as Boolean
    ? ">>> loadingIndacator: onKeyEvent("key", "press")"
    result = true
    if press 
        return result
    end if
